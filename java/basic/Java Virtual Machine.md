...
​		**强引用**：
指在代码中普遍存在的，类似“Object obj = new Object()”这类引用，只要强引用还存在，垃圾收集器永远不会回收被引用的对象

​		**软引用：**
指有用的，但是非必须的对象，内存将要发生溢出异常之前，将会把这些对象列进回收范围中并进行第二次回收

​		**弱引用：**
无论内存是否足够，直接回收

​		**虚引用：**
为对象设置虚引用关联的目的就是希望这个对象被回收时收到一个系统通知。
